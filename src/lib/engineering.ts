...
export type CircuitLoad = { pointId: string; label: string; kind: PlanPoint["kind"]; power: number; voltage: number };
...
export type CircuitAnalysis = {
  ...
  conductorMaterial: Circuit["conductorMaterial"];
  routeLengthOverrideM: number | null;
  powerFactor: number;
};

export type EngineeringOverview = { circuits: CircuitAnalysis[]; unassigned: PlanPoint[]; installedPower: number; demandPower: number; warnings: string[] };
...
export function analyzeCircuit(id: string, points: PlanPoint[], circuit?: Circuit): CircuitAnalysis {
  const loads = points.map((p) => ({ pointId: p.id, label: p.label, kind: p.kind, power: Math.max(0, p.power || 0), voltage: Math.max(0, p.voltage || 0) }));
  ...
  const powerFactor = Math.max(0.01, Math.min(1, circuit?.powerFactor ?? 1));
  const designCurrent = voltage && !mixedVoltage ? demandPower / (voltage * powerFactor) : null;
  ...
  return {
    ...
    installationMethod: circuit?.installationMethod ?? "configurar",
    ambientCorrection: circuit?.ambientCorrection ?? 1,
    groupingCorrection: circuit?.groupingCorrection ?? 1,
    conductorMaterial: circuit?.conductorMaterial ?? "copper",
    routeLengthOverrideM: circuit?.routeLengthOverrideM ?? null,
    powerFactor,
  };
}
...
export function analyzeProject(doc: PlanDocument): EngineeringOverview {
  ...
  for (const point of doc.points) {
    ...
    if (!circuit) { unassigned.push(point); continue; }
  ...
  const circuits = [...allIds].map((id) => analyzeCircuit(id, assigned.get(id) ?? [], definitions.find((c) => c.id === id))).sort((a, b) => a.id.localeCompare(b.id, "pt-BR", { numeric: true }));
  ...
  for (const circuit of circuits) {
    ...
    if (circuit.installationMethod === "configurar") warnings.push(`${circuit.id} ainda não possui método de instalação definido.`);
  }

  return { circuits, unassigned, installedPower: doc.points.reduce((sum, point) => sum + Math.max(0, point.power || 0), 0), demandPower: circuits.reduce((sum, circuit) => sum + circuit.demandPower, 0), warnings };
}

export function nextCircuitId(doc: PlanDocument) {
  const numbers = analyzeProject(doc).circuits.map((c) => /^C(\d+)$/i.exec(c.id)).filter((match): match is RegExpExecArray => Boolean(match)).map((match) => Number(match[1]));

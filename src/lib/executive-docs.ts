...
function commandPath(count: number) {
  if (count <= 1) return { components: ["Fase", "Interruptor simples", "Retorno", "Luminária", "Neutro", "PE"], note: "Comando simples." };
  if (count === 2) return { components: ["Fase", "Paralelo A", "Viajante 1", "Viajante 2", "Paralelo B", "Retorno", "Luminária", "Neutro", "PE"], note: "Comando em dois locais (three-way/paralelo)." };
  const intermediates = Array.from({ length: Math.max(1, count - 2) }, (_, i) => `Intermediário ${i + 1}`);
  return { components: ["Fase", "Paralelo A", ...intermediates.flatMap((name) => ["Viajantes", name]), "Paralelo B", "Retorno", "Luminária", "Neutro", "PE"], note: `Comando em ${count} locais com paralelos nas extremidades e intermediário(s).` };
}
...
export function generateMultilineData(doc: PlanDocument): MultiLineBranch[] {
  ...
  for (const light of lights) {
    ...
    const plan = commandPath(switches.length);
    branches.push({ circuitId: light.circuit || "SEM-CIRCUITO", loadLabel: light.label, path: plan.components, note: plan.note });
  }
  return branches;
}
...
export function generateExecutiveHtml(doc: PlanDocument, title = "Projeto elétrico") {
  ...
  const multiline = generateMultilineData(doc);
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>${escapeHtml(title)}</title><style>body{font-family:Arial,sans-serif;margin:32px;color:#111}h1,h2{margin:0 0 12px}section{page-break-after:always;margin-bottom:32px}table{border-collapse:collapse;width:100%;font-size:12px}th,td{border:1px solid #999;padding:6px;text-align:left}pre{white-space:pre-wrap;font-size:11px}</style></head><body><h1>${escapeHtml(title)}</h1><p>Documento executivo gerado pelo Voltplan. Conferência final e responsabilidade técnica permanecem com profissional habilitado.</p><section><h2>Índice de pranchas</h2>${sheets.map((s) => `<div>${s.code} — ${s.title}</div>`).join("")}</section><section><h2>Quadro de cargas</h2><table><thead><tr><th>Circuito</th><th>Descrição</th><th>Potência</th><th>Tensão</th><th>Corrente</th><th>Cabo</th><th>Disjuntor</th></tr></thead><tbody>${rows}</tbody></table></section><section><h2>Diagrama unifilar</h2><pre>${escapeHtml(generateSingleLineText(doc))}</pre></section><section><h2>Diagrama multifilar</h2>${multiline.map((m) => `<div><b>${escapeHtml(m.circuitId)} · ${escapeHtml(m.loadLabel)}</b><pre>${escapeHtml(m.path.join(" → "))}</pre><small>${escapeHtml(m.note)}</small></div>`).join("") || "<p>Sem comandos suficientes para gerar multifilar.</p>"}</section><section><h2>Legenda</h2>${legend.map((l) => `<div><b>${escapeHtml(l.symbol)}</b> — ${escapeHtml(l.label)}</div>`).join("")}</section><section><h2>Memorial descritivo</h2><pre>${escapeHtml(memorial)}</pre></section></body></html>`;
}

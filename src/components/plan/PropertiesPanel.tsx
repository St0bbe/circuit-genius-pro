...
export function PropertiesPanel({ doc, selection, summary, onChange, onSelect }: Props) {
  ...
  return <div className="flex h-full flex-col overflow-y-auto border-l border-border bg-sidebar p-3">
    ...
    {point && <div className="space-y-3">
      ...
      <Field label="Identificação"><Input value={point.label} onChange={(e) => onChange((d) => ({ ...d, points: d.points.map((p) => p.id === point.id ? { ...p, label: e.target.value } : p) }))} /></Field>
      <div className="grid grid-cols-2 gap-2">
        <Field label="Potência (VA)"><Input type="number" value={point.power} onChange={(e) => onChange((d) => ({ ...d, points: d.points.map((p) => p.id === point.id ? { ...p, power: Number(e.target.value) || 0 } : p) }))} /></Field>
        <Field label="Tensão (V)"><Input type="number" value={point.voltage} onChange={(e) => onChange((d) => ({ ...d, points: d.points.map((p) => p.id === point.id ? { ...p, voltage: Number(e.target.value) || 0 } : p) }))} /></Field>
        <Field label="Altura (m)"><Input type="number" step="0.05" value={point.height} onChange={(e) => onChange((d) => ({ ...d, points: d.points.map((p) => p.id === point.id ? { ...p, height: Number(e.target.value) || 0 } : p) }))} /></Field>
        <Field label="Circuito"><Input placeholder="C01" value={point.circuit} onChange={(e) => onChange((d) => ({ ...d, points: d.points.map((p) => p.id === point.id ? { ...p, circuit: e.target.value.toUpperCase() } : p) }))} /></Field>
        <Field label="Rotação"><Input type="number" step="90" value={point.rotation ?? 0} onChange={(e) => onChange((d) => ({ ...d, points: d.points.map((p) => p.id === point.id ? { ...p, rotation: Number(e.target.value) || 0 } : p) }))} /></Field>
      </div>
